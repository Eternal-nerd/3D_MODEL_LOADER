...
------------------------------INITIALIZATION-----------------------------------
-----------------------------------------------------------------------------*/
void Synchro::setDvcePtr(const Dvce& dvce) {
    dvcePtr_ = &dvce;
}
...
void Synchro::createSyncObjects() {
    ...
    for (size_t i = 0; i < maxFramesInFlight_; i++) {
        if (vkCreateSemaphore(dvcePtr_->getLogical(), &semaphoreInfo, nullptr,
            ...
            vkCreateFence(dvcePtr_->getLogical(), &fenceInfo, nullptr, &inFlightFences_[i]) !=
            VK_SUCCESS) {
...
-----------------------------GETTERS-------------------------------------------
-----------------------------------------------------------------------------*/
const std::vector<VkSemaphore>& Synchro::getImageAvailableSemaphores() const { return imageAvailableSemaphores_; }
const std::vector<VkSemaphore>& Synchro::getRenderFinishedSemaphores() const { return renderFinishedSemaphores_; }
const std::vector<VkFence>& Synchro::getInFlightFences() const { return inFlightFences_; }
...
void Synchro::cleanup() {
    ...
    for (size_t i = 0; i < maxFramesInFlight_; i++) {
        vkDestroySemaphore(dvcePtr_->getLogical(), renderFinishedSemaphores_[i], nullptr);
        vkDestroySemaphore(dvcePtr_->getLogical(), imageAvailableSemaphores_[i], nullptr);
