...
------------------------------INITIALIZATION-----------------------------------
-----------------------------------------------------------------------------*/
void Txtr::setDvcePtr(const Dvce& dvce) {
	dvcePtr_ = &dvce;
}

void Txtr::setCmdrPtr(Cmdr& cmdr) {
	cmdrPtr_ = &cmdr;
}
...
void Txtr::createTextureImage() {
	...
	stbi_uc* pixels = stbi_load(filename_, &texWidth, &texHeight,
	...
	util::createBuffer(imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
		...
		stagingBuffer, stagingBufferMemory, dvcePtr_->getLogical(), dvcePtr_->getPhysical());

	void* data;
	vkMapMemory(dvcePtr_->getLogical(), stagingBufferMemory, 0, imageSize, 0, &data);
	...
	util::createImage(
		...
		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureImage_, textureImageMemory_, dvcePtr_->getLogical(), dvcePtr_->getPhysical());
...
void Txtr::createTextureImageView() {
	...
	textureImageView_ = util::createImageView(textureImage_, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, dvcePtr_->getLogical());
}
...
void Txtr::createTextureSampler() {
	...
	if (vkCreateSampler(dvcePtr_->getLogical(), &samplerInfo, nullptr, &textureSampler_) !=
		VK_SUCCESS) {
...
------------------------------HELPERS------------------------------------------
-----------------------------------------------------------------------------*/
void Txtr::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout) {
...
void Txtr::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
	VkCommandBuffer commandBuffer = cmdrPtr_->beginSingleTimeCommands();
