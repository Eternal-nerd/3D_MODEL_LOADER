...
------------------------------INITIALIZATION-----------------------------------
-----------------------------------------------------------------------------*/
void Cmdr::setDvcePtr(const Dvce& dvce) {
	dvcePtr_ = &dvce;
}

void Cmdr::createPool() {
	util::log("Creating command pool...");
	QueueFamilyIndices queueFamilyIndices = util::findQueueFamilies(dvcePtr_->getPhysical(), dvcePtr_->getSurface());
	...
	if (vkCreateCommandPool(dvcePtr_->getLogical(), &poolInfo, nullptr, &commandPool_) !=
		VK_SUCCESS) {
